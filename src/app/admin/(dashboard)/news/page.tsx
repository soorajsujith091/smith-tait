      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[var(--color-navy)] rounded-2xl p-6 border border-white/5">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-display text-white">Published Articles</h2>
          </div>
          
          <div className="space-y-4">
            {/* Mock Article List */}
            {["Illuminating the Future: 2026 Trends", "Smith Tait Wins MENA Design Award", "The Psychology of Light in Hospitality"].map((article, i) => (
              <div key={i} className="group bg-[var(--color-ink-dark)] border border-white/5 rounded-xl p-4 flex items-center justify-between hover:border-[var(--color-accent)]/30 transition-colors">
                <div className="flex items-center gap-4">
                   <div className="w-16 h-12 bg-white/10 rounded overflow-hidden relative">
                      <img src={`/images/general/park-city.jpg`} alt="thumbnail" className="w-full h-full object-cover opacity-50" />
                   </div>
                   <div>
                     <h3 className="font-display text-white text-sm">{article}</h3>
                     <p className="font-body text-[10px] text-[var(--color-grey)] uppercase tracking-wider mt-1">Published • {new Date().toLocaleDateString()}</p>
                   </div>
                </div>
